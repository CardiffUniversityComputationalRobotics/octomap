...
namespace social_octomap
{
  ...
  void ViewerGui::addOctree(social_octomap::AbstractOcTree *tree, int id, social_octomap::pose6d origin)
  {
    // is id in use?
    OcTreeRecord *r;
    ...
    if (foundRecord && r->octree->getTreeType().compare(tree->getTreeType()) != 0)
    {
      ...
      {
        SOCIAL_OCTOMAP_ERROR("Could not create drawer for tree type %s\n", tree->getTreeType().c_str());
      }
  ...
  void ViewerGui::addOctree(social_octomap::AbstractOcTree *tree, int id)
  {
    social_octomap::pose6d o; // initialized to (0,0,0) , (0,0,0,1) by default
    addOctree(tree, id, o);
  }
  ...
  void ViewerGui::showOcTree()
  {
    ...
    for (std::map<int, OcTreeRecord>::iterator it = m_octrees.begin(); it != m_octrees.end(); ++it)
    {
      ...
      social_octomap::point3d pmin(lminX, lminY, lminZ);
      social_octomap::point3d pmax(lmaxX, lmaxY, lmaxZ);
    ...
    QString memory = QString("Single node: %L1 B; ").arg(memorySingleNode) + QString("Octree: %L1 B (%L2 MB)").arg(memoryUsage).arg((double)memoryUsage / (1024. * 1024.), 0, 'f', 3);
  ...
  void ViewerGui::generateOctree()
  {
    ...
    if (m_scanGraph)
    {
      ...
      OcTree *tree = new social_octomap::OcTree(m_octreeResolution);

      social_octomap::ScanGraph::iterator it;
  ...
  void ViewerGui::gotoFirstScan()
  {
    if (m_scanGraph)
    {
      ...
      // if (m_ocTree) delete m_ocTree;
      // m_ocTree = new social_octomap::OcTree(m_octreeResolution);
      OcTree *tree = new social_octomap::OcTree(m_octreeResolution);
      this->addOctree(tree, DEFAULT_OCTREE_ID);
  ...
  void ViewerGui::openGraph(bool completeGraph)
  {
    ...
    if (m_scanGraph)
      delete m_scanGraph;
    m_scanGraph = new social_octomap::ScanGraph();
  ...
  {
    OcTree *tree = new social_octomap::OcTree(m_filename);
    this->addOctree(tree, DEFAULT_OCTREE_ID);
  ...
  void ViewerGui::openMapCollection()
  {
    ...
    SOCIAL_OCTOMAP_DEBUG("Opening hierarchy from %s...\n", m_filename.c_str());
    ...
    {
      SOCIAL_OCTOMAP_DEBUG("Adding hierarchy node %s\n", (*it)->getId().c_str());
      OcTree *tree = (*it)->getMap();
      if (!tree)
        SOCIAL_OCTOMAP_ERROR("Error while reading node %s\n", (*it)->getId().c_str());
      else
      {
        SOCIAL_OCTOMAP_DEBUG("Read tree with %zu tree nodes\n", tree->size());
      }
  ...
  void ViewerGui::on_actionSettings_triggered()
  {
    ...
    if (dialog.exec())
    {
      ...
      if (oldType != m_laserType)
      { // parameters changed, reload file:
        openFile();
      }
  ...
  void ViewerGui::on_actionDelete_nodes_outside_of_selection_triggered()
  {
    ...
    for (std::map<int, OcTreeRecord>::iterator t_it = m_octrees.begin(); t_it != m_octrees.end(); ++t_it)
    {
      ...
      if (octree)
      {
        social_octomap::OcTreeKey minKey, maxKey;
  ...
  void ViewerGui::on_actionHideBackground_toggled(bool checked)
  {
    ...
    {
      if (checked)
        m_glwidget->removeSceneObject(r->octree_drawer);
      else
        m_glwidget->addSceneObject(r->octree_drawer);
      m_glwidget->update();
    }
  }
  ...
  void ViewerGui::voxelSelected(const QMouseEvent *e)
  {
    ...
    std::list<social_octomap::OcTreeVolume> selection;
    ...
    for (std::map<int, OcTreeRecord>::iterator it = m_octrees.begin(); it != m_octrees.end(); ++it)
    {
      ...
      {
        SOCIAL_OCTOMAP_ERROR("Could not select nodes of this tree type %s\n", tree->getTreeType().c_str());
        continue;
      }
      if (ray_hit)
      {
        ...
        it->second.octree_drawer->clearOcTreeSelection();
    }
