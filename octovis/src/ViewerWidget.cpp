...
namespace social_octomap
{
  ...
  ViewerWidget::ViewerWidget(QWidget *parent) : QGLViewer(parent), m_zMin(0.0), m_zMax(1.0)
  {
  ...
  {
    QString help = "<h2>SocialOctomap 3D viewer</h2>";

    help += "The SocialOctomap library implements a 3D occupancy grid mapping approach. "
            ...
            "<br/><br/>"
            "SocialOctomap is available at https://social_octomap.github.io, and is actively "
  ...
  {
    // copying social_octomap::Quaternion parameters to qglviewer::Quaternion does not work (reason unknown)
  ...
  void ViewerWidget::appendCurrentToCameraPath(int id)
  {
    int frame = 0;
    if (camera()->keyFrameInterpolator(id))
      frame = camera()->keyFrameInterpolator(id)->numberOfKeyFrames();
  ...
  void ViewerWidget::cameraPathInterpolated()
  {
    ...
    if (kfi)
    {
      ...
      {
        if (kfi->keyFrameTime(current_frame) <= kfi->interpolationTime())
          current_frame = i;
        else
          break;
      }
