...
namespace social_octomap
{
  ...
  void PointcloudDrawer::setScanGraph(const ScanGraph &graph)
  {
    ...
    // count points first:
    for (social_octomap::ScanGraph::const_iterator it = graph.begin(); it != graph.end(); it++)
    {
    ...
    unsigned i = 0;
    for (social_octomap::ScanGraph::const_iterator graph_it = graph.begin(); graph_it != graph.end(); graph_it++)
    {
      social_octomap::Pointcloud *scan = new Pointcloud((*graph_it)->scan);
