...
namespace social_octomap
{
                                 ...
                                 octree_grid_vertex_size(0), m_alphaOccupied(0.8), map_id(0)
  {
    ...
    // origin and movement
    initial_origin = social_octomap::pose6d(0, 0, 0, 0, 0, 0);
  ...
  void OcTreeDrawer::draw() const
  {
    ...
    if (!m_alternativeDrawing && gl_list_index != -1)
    { // Free video card memory
    ...
    if (m_update || !m_alternativeDrawing)
    {
      ...
      // push current status
      glPushMatrix();
      // social_octomap::pose6d relative_transform = origin * initial_origin.inv();

      social_octomap::pose6d relative_transform = origin; // * initial_origin;
  ...
  void OcTreeDrawer::setOcTree(const AbstractOcTree &in_tree, const pose6d &in_origin, int map_id_)
  {
    ...
    this->initial_origin = social_octomap::pose6d(social_octomap::point3d(0, 0, 0), in_origin.rot());
    ...
    bool uses_origin = ((origin.rot().x() != 0.) && (origin.rot().y() != 0.) && (origin.rot().z() != 0.) && (origin.rot().u() != 1.));
         ...
         it != end; ++it)
    {
      ...
      { // inner node voxels (for grid structure only)
        if (showAll)
        {
          if (uses_origin)
            voxel = OcTreeVolume(origin.rot().rotate(it.getCoordinate()), it.getSize());
  ...
  void OcTreeDrawer::setOcTreeSelection(const std::list<social_octomap::OcTreeVolume> &selectedVoxels)
  {
    m_update = true;
  ...
  unsigned int OcTreeDrawer::generateCube(const social_octomap::OcTreeVolume &v,
  ...
  unsigned int OcTreeDrawer::setCubeColorHeightmap(const social_octomap::OcTreeVolume &v,
                                                   const unsigned int &current_array_idx,
  ...
  void OcTreeDrawer::generateCubes(const std::list<social_octomap::OcTreeVolume> &voxels,
                                   ...
                                   GLfloat **glColorArray)
  {
    ...
    for (std::list<social_octomap::OcTreeVolume>::const_iterator it = voxels.begin();
         it != voxels.end(); it++)
    {
  ...
  void OcTreeDrawer::initOctreeGridVis()
  {
    ...
    std::list<social_octomap::OcTreeVolume>::iterator it_rec;
    unsigned int i = 0;
    double x, y, z;
  ...
  void OcTreeDrawer::setOrigin(social_octomap::pose6d t)
  {
    origin = t;
  ...
  void OcTreeDrawer::drawAxes() const
  {
    ...
    double scale = sqrt(initial_origin.rot().x() * initial_origin.rot().x() + initial_origin.rot().y() * initial_origin.rot().y() + initial_origin.rot().z() * initial_origin.rot().z());
    double ax = initial_origin.rot().x() / scale;
    double ay = initial_origin.rot().y() / scale;
    double az = initial_origin.rot().z() / scale;

    if (angle > 0)
      glRotatef(OTD_RAD2DEG * angle, ax, ay, az);
