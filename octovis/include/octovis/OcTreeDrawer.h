...
namespace social_octomap
{
  ...
  class OcTreeDrawer : public social_octomap::SceneObject
  {
  public:
    OcTreeDrawer();
    ...
    virtual void setOcTree(const AbstractOcTree &octree, const social_octomap::pose6d &origin, int map_id_);

    // modification of existing drawer  ------------------

    /// sets a new selection of the current OcTree to be drawn
    void setOcTreeSelection(const std::list<social_octomap::OcTreeVolume> &selectedPoints);
    ...
    void setOrigin(social_octomap::pose6d t);
    void enableAxes(bool enabled = true)
    {
      m_update = true;
      m_displayAxes = enabled;
    };
  ...
  protected:
    ...
    void generateCubes(const std::list<social_octomap::OcTreeVolume> &voxels,
    ...
    unsigned int generateCube(const social_octomap::OcTreeVolume &v,
                              const std::vector<octomath::Vector3> &cube_template,
                              const unsigned int &current_array_idx,
                              GLfloat ***glArray);
    unsigned int setCubeColorHeightmap(const social_octomap::OcTreeVolume &v,
                                       const unsigned int &current_array_idx,
    ...
    std::list<social_octomap::OcTreeVolume> m_grid_voxels;
    ...
    social_octomap::pose6d origin;
    social_octomap::pose6d initial_origin;
