...
#include <social_octomap/math/Utils.h>
#include "testing.h"
                        ...
                        point3d &child_center)
{
  // x-axis
  if (pos & 1)
    child_center(0) = parent_center(0) + center_offset;
  else
    child_center(0) = parent_center(0) - center_offset;

  // y-axis
  if (pos & 2)
    child_center(1) = parent_center(1) + center_offset;
  else
    child_center(1) = parent_center(1) - center_offset;
  // z-axis
  if (pos & 4)
    child_center(2) = parent_center(2) + center_offset;
  else
    child_center(2) = parent_center(2) - center_offset;
}
...
bool OcTreeVolumeSortPredicate(const OcTreeVolume &lhs, const OcTreeVolume &rhs)
{
  return (lhs.second < rhs.second || (lhs.second == rhs.second &&
                                      lhs.first.x() < rhs.first.x() && lhs.first.y() < rhs.first.y() && lhs.first.z() < rhs.first.z()));
}
...
void boundingBoxTest(OcTree *tree)
{
  ...
  social_octomap::point3d bbxMin = social_octomap::point3d(float(temp_x), float(temp_y), float(temp_z));

  tree->getMetricMax(temp_x, temp_y, temp_z);
  social_octomap::point3d bbxMax = social_octomap::point3d(float(temp_x), float(temp_y), float(temp_z));
  ...
  for (OcTree::leaf_iterator it = tree->begin(), end = tree->end(); it != end; ++it)
  {
    ...
    if (key[0] >= bbxMinKey[0] && key[0] <= bbxMaxKey[0] && key[1] >= bbxMinKey[1] && key[1] <= bbxMaxKey[1] && key[2] >= bbxMinKey[2] && key[2] <= bbxMaxKey[2])
    {
...
int main(int argc, char **argv)
{
  ...
  social_octomap::point3d ptMinBBX(-0.1, -0.1, -0.1);
  social_octomap::point3d ptMaxBBX(0.1, 0.1, 0.1);
  ...
  social_octomap::OcTreeKey minBBX(10, 10, 16);
  social_octomap::OcTreeKey maxBBX(10, 10, 18);
   ...
   */
  point3d tree_center;
  tree_center(0) = tree_center(1) = tree_center(2) = (float)(((double)tree_max_val) * tree->getResolution());
